...
from docuware import conn, organization, structs, types, errors, utils
...
class DocuwareClient(types.DocuwareClientP):
    def __init__(self, url: str, verify_certificate: bool = True):
        self.conn = conn.Connection(
            url, case_insensitive=True, verify_certificate=verify_certificate
        )
    ...
    def organizations(self) -> Generator[types.OrganizationP, None, None]:
        ...
        return (
            organization.Organization(org, self)
            for org in result.get("Organization", [])
        )
    ...
    def organization(
        ...
        return structs.first_item_by_id_or_name(
            self.organizations, key, required=required
        )
